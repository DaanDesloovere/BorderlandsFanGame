...
@export var type : Constants.AmmoType
@onready var Ammo : Node3D = $".."
...
func _on_body_entered(body: Node3D) -> void:
	if body.is_in_group("Player") and not PickedUp and body.AmmoArray[type] < body.MaxAmmoArray[type]:
		...
		tween.tween_property(Ammo, "global_position", endPosition, 0.3).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
